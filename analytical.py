...
def compute_implicit_icl_updates(
    ...
    """Computes implicit in-context learning (ICL) updates according to 
    ...
    more accurate skip connections. Note that the weight update is more involved 
    ...
    Args:
        ...
        use_layer_norm: whether to assume layer normalisation
    ...
    A_C_x = block.attention_layer(C_x)
    
    # A(x): attention output with query only (no context) → (B, 1, D)
    A_x = block.attention_layer(x)
    ...
    if use_layer_norm:
        if use_skips:
            A_C_x = jax.vmap(jax.vmap(ln_2))(A_C_x + C_x)
            A_x = jax.vmap(jax.vmap(ln_2))(A_x_broadcasted + x)
        else:
            A_C_x = jax.vmap(jax.vmap(ln_2))(A_C_x)
            A_x = jax.vmap(jax.vmap(ln_2))(A_x_broadcasted)
